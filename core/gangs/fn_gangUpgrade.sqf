...
if (_action) then {
    if (life_KavaBank < _upgradePrice) exitWith {
        hint parseText format[
            ...
            [life_KavaBank] call life_fnc_numberText,
            [_upgradePrice - life_KavaBank] call life_fnc_numberText
        ];
    };
    life_KavaBank = life_KavaBank - _upgradePrice;
    ...
    if (life_HC_isActive) then {
        [2,group player] remoteExecCall ["HC_fnc_updateGang",HC_Life];
    } else {
