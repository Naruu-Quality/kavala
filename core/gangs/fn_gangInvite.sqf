...
if (_action) then {
    ...
    if (life_HC_isActive) then {
        ...
        [5,_group] remoteExecCall ["TON_fnc_updateGang",RSERV];
    };
} else {
    ...
    if (life_HC_isActive) then {
        [5,_grpMembers] remoteExecCall ["HC_fnc_updateGang",HC_Life];
    } else {
        [5,_grpMembers] remoteExecCall ["TON_fnc_updateGang",RSERV];
    };
