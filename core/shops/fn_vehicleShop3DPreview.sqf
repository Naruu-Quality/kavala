...
if (_classView != "" && {isClass (configFile >> "CfgVehicles" >> _classView) && {getNumber (configFile >> "CfgVehicles" >> _classView >> "scope") > 0}}) then
{
	...
	if (isNull life_preview_3D_vehicle_object || {_classView != typeOf life_preview_3D_vehicle_object}) then
	{
		...
		if (!life_pos_exist) then {
			...
			life_pos_exist = true;
			life_activeObj = "#lightpoint" createVehicle life_pos_attach;
			life_activeObj setlightbrightness 0.5;
			life_activeObj setlightcolor [1,1,1];
			life_activeObj setlightambient [1,1,1];
		};
		...
		[life_activeObj] remoteExecCall ["life_fnc_hideObj",-_id];
