...
if(isNil {_house getVariable "house_owner"}) exitWith {hint parseText "There is no owner for this house."};
closeDialog 0;
...
if(_action) then {
	...
	life_KavaBank = life_KavaBank + (round((_houseCfg select 0)/2));
	...
	_containers = [getPosATL _house, ["Box_IND_Grenades_F","B_supplyCrate_F","Land_Sink_F","CargoNet_01_barrels_F","Land_OfficeCabinet_01_F","Fridge_01_closed_F"], 9] call life_fnc_nearestObjects;
