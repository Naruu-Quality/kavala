...
switch(true) do {
	...
	case ((typeOf _container) == "Land_Sink_F") : {_type = "illegalbox"};
	case ((typeOf _container) == "CargoNet_01_barrels_F") : {_type = "illegalboxbig"};
	case ((typeOf _container) == "Land_OfficeCabinet_01_F") : {_type = "armoire"};
	case ((typeOf _container) == "Fridge_01_closed_F") : {_type = "frigo"};
	default {_type = ""};
};
			...
			if(count _containers >= (_houseCfg select 1)) then {
				...
				hint localize "STR_ISTR_Box_HouseFull";
					...
					if(life_HC_isActive) then {
						[_uid,_container] remoteExecCall ["HC_fnc_addContainer",HC_Life];
					} else {
