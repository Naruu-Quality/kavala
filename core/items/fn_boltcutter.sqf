...
if(((nearestObject [[16019.5,16952.9,0],"Land_Dome_Big_F"]) == _building OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _building) && ({_x getVariable ["rank",0] > 0} count playableUnits < (LIFE_SETTINGS(getNumber,"cops_online_min")))) exitWith {
...
if((nearestObject [[16019.5,16952.9,0],"Land_Dome_Big_F"]) == _building OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _building) then {
	...
	if(_building getVariable ["house_alarm",false])then
	{
		...
		{
			if(((getPlayerUID _x) == ((_building getVariable "house_owner") select 0)) && (side _x == civilian)) exitWith {
