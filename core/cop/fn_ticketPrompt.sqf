...
[] spawn {
	...
	if(isNull (findDisplay 2600) && !life_ticket_paid) then {
		[[0,7],"STR_Cop_Ticket_Refuse",true,[profileName]] remoteExecCall ["life_fnc_broadcast",west];
		[[0,7],"STR_Cop_Ticket_Refuse",true,[profileName]] remoteExecCall ["life_fnc_broadcast",life_ticket_cop];
		[format["[AMENDE.REFUSE] %1(%2) -A REFUSER DE PAYER L AMENDE",name player, getPlayerUID player]] remoteExecCall ["A3Log", 2];
	};
};