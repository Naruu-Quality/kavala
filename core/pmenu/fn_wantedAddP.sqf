...
if((lbCurSel 2406) == -1) exitWith {hint parseText "You must select a perp."};
if((lbCurSel 2407) == -1) exitWith {hint parseText "You must select a crime."};
_unit = lbData [2406,lbCurSel 2406];
_unit = call compile format["%1",_unit];
_amount = lbData [2407,lbCurSel 2407];
if(isNil "_unit") exitWith {};
//if(side _unit == west) exitWith {hint parseText "What are ya trying to do, start a police war? Dickhead." };
//if(_unit == player) exitWith {hint parseText "You can't make yourself wanted, dipshit";};
